...
```bash
# Docker 실행
docker run -d --name aisogething-db -p 5432:5432 \
  -e POSTGRES_USER=postgres \
  -e POSTGRES_PASSWORD=0000 \
  -e POSTGRES_DB=aisogething \
...
- "Performance via Simplicity" — ORM의 불필요한 추상화를 제거하고 SQL 본연의 강력함을 그대로 사용한다.
- 단, DB Connection Pool, Transaction Management 같은 어려운 일은 검증된 엔진에게 맡긴다.

### 2. Technical Stack

- **Engine**: SQLAlchemy Core (Sync) — Connection Pool 관리가 탁월함
- **Query**: Raw SQL (`text`) — ORM 변환 비용 Zero
- **Interface**: Custom Wrapper (`execute`, `fetch_one`, `fetch_all`)

### 3. Wrapper Functions

- `execute(sql, params)`: 자동으로 Transaction을 시작하고, 성공 시 즉시 Commit, 실패 시 Rollback
- `fetch_one(sql, params)`: 결과 Row를 즉시 Python Dictionary로 변환하여 반환
- `fetch_all(sql, params)`: 대량 조회 시 `List[Dictionary]` 구조로 가볍게 반환
...
- 현재 규모(동시접속 < 1,000)에서는 Async의 오버헤드보다 Sync의 직관성과 디버깅 용이성이 더 큰 가치를 가짐
