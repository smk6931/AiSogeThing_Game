...
// 라인 기반 Zone 메쉬 (도로 등)
const ZoneLineMesh = React.memo(({ geometry, color, opacity, elevation, useStencil, renderOrder = 101 }) => {
  if (!geometry) return null;
  return (
    <mesh position={[0, elevation, 0]} renderOrder={renderOrder}>
      <primitive object={geometry} attach="geometry" />
      <meshBasicMaterial
        color={color}
        transparent={true}
        ...
        side={THREE.DoubleSide}
        depthWrite={false}
        depthTest={false} // 도로면 무조건 최상단 강제
        stencilWrite={useStencil}
        stencilRef={1} // 통일
...
const ZoneMask = ({ maskArea, elevation }) => {
  const geo = useMemo(() => {
  ...
  return (
    <mesh geometry={geo} rotation={[-Math.PI / 2, 0, 0]} position={[0, elevation, 0]} renderOrder={0}>
      <meshBasicMaterial
        colorWrite={false}
        depthWrite={false}
        depthTest={false}
