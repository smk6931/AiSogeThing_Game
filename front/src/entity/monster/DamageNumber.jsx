...
export const DamageNumber = ({ id, damage, position, onRemove, color: colorOverride, outlineColor: outlineColorOverride }) => {
  ...
  // 데미지 크기에 따라 색상/폰트 구분 (override 가능)
  const isBig = damage >= 25;
  const color = colorOverride ?? (isBig ? '#ff8800' : '#ffffff');
  const outlineColor = outlineColorOverride ?? '#000000';
